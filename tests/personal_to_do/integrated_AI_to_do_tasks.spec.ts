...
test.describe.skip("Personal To Do functionality", () => {
    ...
    test("[test-063] Integrated AI component in personal to do tasks functionality. New feature ticket: 44655249514",
        async ({page}) => {
            const todoPage = new TodoPage(page);
            const imageName = 'Enter-Tasks-To-Do-Example.jpg';

            await todoPage.openToDoFullView();
            await todoPage.clickAddNewTodoItem();
            await todoPage.addNewTaskAndUploadImage(imageName);
            await todoPage.analyseDataAndValidateItShowsCorrectly();
            await todoPage.deleteNewlyCreatedTask();

            await todoPage.clickAddNewTodoItem();
            await todoPage.addNewTaskAndParseText();
            await todoPage.analyseDataAndValidateItShowsCorrectly();
        });
});