...
test.describe("Image based templates signature field's test", () => {
    ...
    test("[test-051] Verify signature field expanding functionality", async ({ page }, testInfo) => {
        const dashboard = new DashboardHelpers(page);
        const signatureHelpers = new SignatureHelpers(page);

        const templateName = "CLIENT SERVICES AGREEMENT – FAMILY";
        ...
        await signatureHelpers.expendSignatureField();
        await signatureHelpers.signExpandedTemplate();
